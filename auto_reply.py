...
from napcat_client import NapCatClient, QQMessage
...
PLAYER_LOG = Path(__file__).parent / "PlayerLog"
...
@dataclass
class Conversation:
    target_key: str  # "private:2712509058" / "group:912389435"
...
class AutoReply:
    ...
    async def _run_message(self, msg: QQMessage):
        ...
        if cmd_match:
            cmd_name, cmd_full = cmd_match  # cmd_name="bot", cmd_full=".bot off"
    ...
    async def _fetch_agents_from_cherrystudio(self, whitelist: list[str] | None = None) -> dict[str, dict]:
        ...
        agents = {}
        all_names: list[tuple[str, str]] = []  # 用于白名单为空时的警告日志
        for item in items:
        ...
        if not whitelist_set:
            logger.warning("未设置 Agent 白名单！以下为各个 Agent 名称和 ID，请将允许的 Agent ID 添加到 agent_whitelist 中：")
            for i, (name, aid) in enumerate(all_names):
                logger.warning(f"  [{i}] {name} -> {aid}")
            logger.warning("请在控制台中输入要启用的 Agent 编号（如 0,1,3 或 0-3），直接回车则全部启用。")
    ...
    async def _summarize(self, log_text: str) -> str:
        llm = self._get_llm_config()
        if not llm.get("api_url") or not llm.get("api_key"):
            return ""
    ...
    def _get_model(self, conv: Conversation | None = None) -> str:
        ...
        if "127.0.0.1" in llm.get("api_url", "") and ":" not in model:
            return f"deepseek:{model}"
        return model
    ...
    def _get_vision_config(self) -> dict:
        ...
        if self._vision_providers:
            cfg = dict(self._vision_providers[self._active_vision_idx])
            cfg["prompt"] = self._vision_prompt
            return cfg
        return {"url": "", "key": "", "model": "", "prompt": self._vision_prompt}

    def _get_llm_config(self) -> dict:
        """获取当前 LLM provider 配置"""
        if self._llm_providers:
            return self._llm_providers[self._active_llm_idx]
    ...
    def _cmd_master_llm_list(self) -> str:
        ...
        lines.append("\n切换指令: .order master LLMChange <模型名>")
        return "\n".join(lines)

    async def _cmd_master_llm_change(self, msg_type: str, target: str, name: str) -> str:
        name = name.strip()
        if not name:
            return "用法: .order master LLMChange <模型名>\n用 .order master LLMList 查看可用模型"
    ...
    def _cmd_master_llm_status(self) -> str:
        ...
        lines = [
            "📊 状态:",
            f"  LLM:   #{self._active_llm_idx} {llm.get('name', '?')} ({llm.get('api_url', '')[:40]}...)",
            f"  Vision:#{self._active_vision_idx} {vis.get('name', '?')} ({vis.get('api_url', '')[:40]}...)",
    ...
    def master_llm_list(self) -> str:
        return self._cmd_master_llm_list()

    async def master_llm_change(self, msg_type: str, target: str, name: str) -> str:
        return await self._cmd_master_llm_change(msg_type, target, name)

    def master_llm_status(self) -> str:
        return self._cmd_master_llm_status()
    ...
    def log_new(self, group_id: str, name: str) -> str:
        ...
        folder_name = f"{group_id}_{name.strip()}_{suffix}"
        ...
        self._active_log_display[group_id] = name.strip()
        self._log_paused.discard(group_id)
        return f"已创建日志「{name.strip()}」并开始记录。"
